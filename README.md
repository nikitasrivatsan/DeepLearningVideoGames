...
In addition to Atari Pong, we also attempted to train our network on a Tetris emulator. Unfortunately, we were unable to train our network for long enough in order to receive human-level performance, although qualitatively we could see that the system was improving both in terms of strategy and in that the Q function was accurately reflecting the optimality of the game state. The most likely reason for this is that the Tetris game score only updates when a level is cleared and when the game ends. During observation and exploration, it is extremely unlikely that we will clear any levels through random actions, so the only reinforcement would come from losing the game. Note that since this corresponds to a terminal state, the loss function would not be defined with respect to the current network weights.

Although our system was able to learn to stack the pieces in such a way that they would not reach the top for a long time, it would have taken several consecutive days of training before it would begin to start clearing levels regularly enough to realize that this was the optimal strategy to maximize score. We unfortunately did not have the time to train for this long, but this remains a future direction that we are interested in exploring outside of the scope of this class. Once again, we would like to reiterate that the method itself should, in theory, still work for Tetris but will simply take more time than Pong to achieve human-level performance.

## 7. Conclusions

We utilized deep Q learning to train a neural network to play Pong and partially to play Tetris from just images of the current game screen and no knowledge of the internal game state. Qualitatively, the trained Pong network appeared to perform near human level. We were unable to train an effective Tetris network. However, we believe that this is most likely an issue of time constraints and not network architecture. Given sufficient time, a human-level performance Tetris network could be trained. 
